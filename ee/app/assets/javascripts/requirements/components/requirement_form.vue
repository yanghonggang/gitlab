...
export default {
  ...
  computed: {
    ...
    canEditRequirement() {
      return this.isCreate ? true : this.canUpdate && !this.isArchived;
    },
  },
  watch: {
    requirement: {
    ...
    drawerOpen(value) {
      ...
      if (!value) {
        ...
        document.addEventListener('keydown', this.handleDrawerKeydown.bind(this));
        debounce(() => {
          this.firstEl = this.$refs.drawerEl.$el.querySelector('.gl-drawer-close-button');
          this.lastEl = this.$refs.drawerEl.$el.querySelector(
            this.enableRequirementEdit ? '.js-requirement-cancel' : '.js-issuable-edit',
          );
        }, 50)();
      }
    },
    enableRequirementEdit(value) {
      this.$nextTick(() => {
        this.lastEl = this.$refs.drawerEl.$el.querySelector(
          value ? '.js-requirement-cancel' : '.js-issuable-edit',
        );
      });
    },
  },
  ...
  methods: {
    ...
    handleDrawerClose() {
      ...
      document.removeEventListener('keydown', this.handleDrawerKeydown.bind(this));
    },
