...
RSpec.describe 'getting compliance frameworks for a collection of projects' do
  ...
  let(:query) do
    graphql_query_for(
      :projects, { ids: project_ids }, query_nodes(:compliance_frameworks)
    )
  end

  let(:single_project_query) do
    graphql_query_for(
      :projects, { ids: [project_ids.first] }, query_nodes(:compliance_frameworks)
    )
  end

  it 'avoids N+1 queries', :use_sql_query_cache do
    query_count = ActiveRecord::QueryRecorder.new(skip_cached: false) { post_graphql(query, current_user: current_user) }.count
    single_project_query_count = ActiveRecord::QueryRecorder.new(skip_cached: false) { post_graphql(single_project_query, current_user: current_user) }.count

    expect(query_count).to eq(single_project_query_count)
  end
