...
RSpec.describe GroupWikiPage::Meta do
  ...
  describe 'Associations' do
    ...
    it 'can find slugs' do
      meta = create(:group_wiki_page_meta)
      slugs = create_list(:group_wiki_page_slug, 3, group_wiki_page_meta: meta)
  ...
  describe 'Validations' do
    subject do
      described_class.new(title: 'some title', group: group)
    end
    ...
    it 'is forbidden to have two records for the same group with the same canonical_slug' do
      ...
      create(:group_wiki_page_meta, canonical_slug: the_slug, group: group)

      in_violation = build(:group_wiki_page_meta, canonical_slug: the_slug, group: group)
  ...
  describe '#canonical_slug' do
    ...
    let_it_be(:meta) do
      described_class.create!(title: generate(:wiki_page_title), group: group)
    end
    ...
    it 'can be set on initialization' do
      meta = create(:group_wiki_page_meta, canonical_slug: 'foo')
    ...
    context 'we have some non-canonical slugs' do
      before do
        create_list(:group_wiki_page_slug, 2, group_wiki_page_meta: subject)
      end
    ...
    context 'we have a canonical slug' do
      ...
      it 'has the correct value' do
        slug = create(:group_wiki_page_slug, :canonical, group_wiki_page_meta: subject)
    ...
    describe 'canonical_slug=' do
      ...
      context 'the slug is already in the DB (but not canonical)' do
        let_it_be(:slug_record) { create(:group_wiki_page_slug, group_wiki_page_meta: meta) }
      ...
      context 'the slug is already in the DB (and canonical)' do
        let_it_be(:slug_record) { create(:group_wiki_page_slug, :canonical, group_wiki_page_meta: meta) }
        let(:slug) { slug_record.slug }
        let(:query_limit) { 4 }
  ...
  describe '.find_or_create' do
    ...
    let(:wiki_page) { create(:wiki_page, container: group) }
    ...
    def create_context
      ...
      create(:group_wiki_page_meta, group: other_group, canonical_slug: wiki_page.slug)
      ...
      create(:group_wiki_page_meta, group: group, canonical_slug: different_slug)
    ...
    shared_examples 'metadata examples' do
      ...
      it 'makes a reasonable number of DB queries' do
        expect(group).to eq(wiki_page.wiki.group)
    ...
    context 'there are problems' do
      ...
      context 'a conflicting record exists' do
        before do
          create(:group_wiki_page_meta, group: group, canonical_slug: last_known_slug)
          create(:group_wiki_page_meta, group: group, canonical_slug: current_slug)
        end
      ...
      context 'the wiki page is not valid' do
        let(:wiki_page) { build(:wiki_page, container: group, title: nil) }
    ...
    context 'we want to change the slug back to a previous version' do
      ...
      let(:wiki_page) { create(:wiki_page, title: slug_1, container: group) }
