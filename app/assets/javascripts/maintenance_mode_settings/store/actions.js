...
export const fetchMaintenanceModeSettings = ({ commit }) => {
  commit(types.REQUEST_MAINTENANCE_MODE_SETTINGS);
  Api.getApplicationSettings()
    .then(({ data }) => {
      commit(types.RECEIVE_MAINTENANCE_MODE_SETTINGS_SUCCESS, {
        maintenanceEnabled: data.maintenance_mode,
        bannerMessage: data.maintenance_mode_message,
      });
    })
    .catch(() => {
      createFlash({ message: __('There was an error fetching the Maintenance Mode Settings') });
      commit(types.RECEIVE_MAINTENANCE_MODE_SETTINGS_ERROR);
    });
};
