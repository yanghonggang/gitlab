...
import reportTypeDownloadPathsQuery from './queries/report_type_download_paths.graphql';
...
export default {
  ...
  apollo: {
    reportArtifacts: {
      query: reportTypeDownloadPathsQuery,
      variables() {
        return {
...
<template>
  ...
  >
    ...
    <template v-if="canShowDownloads" #action-buttons>
      <security-report-download-dropdown :artifacts="reportArtifacts" :loading="$apollo.loading" />
    </template>
  </report-section>
